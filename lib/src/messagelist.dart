...
class messagelist extends StatefulWidget {
  dynamic info;
  var width = MediaQueryData.fromWindow(WidgetsBinding.instance.window)
      .size
      .width
      .toInt();
...
class _messagelist extends State<messagelist> {
  ...
  void getlist(int mins) async {
    var re = json.decode(await APIs().getlist_member(
        widget.info['token'], type, Category, ItemID, pagecounter));
    ...
    for (int i = 0; i < re['projectLists'].length; i++) {
      ...
      goinfo.buildinfo =
          re['projectLists'][i]['repair']['building'].toString() +
              '棟' +
              re['projectLists'][i]['repair']['household'].toString() +
              '戶' +
              re['projectLists'][i]['repair']['floor'].toString() +
              '樓';
      ...
      if (re['projectLists'][i]['handlers'].length > 0) {
        ...
        goinfo.handler_message =
            re['projectLists'][i]['handlers'][0]['message'];
        goinfo.handler_createManagerName =
            re['projectLists'][i]['handlers'][0]['createManagerName'];
        for (int j = 0;
            j < re['projectLists'][i]['handlers'][0]['images'].length;
            j++) {
          goinfo.handler_img1[j] =
              re['projectLists'][i]['handlers'][0]['images'][j]['url'];
        }
      }
  ...
  void getPostsData(var list, int mins) {
    ...
    for (int i = 0; i < list.length; i++) {
      ...
      String show = '';
      if (list[i].deal_status == '案件結案') {
        show = list[i].deal_status;
      } else if (list[i].deal_status == '業外處理') {
        show = list[i].deal_status;
      } else if (list[i].deal_status == '會員自行處理') {
        show = list[i].deal_status;
      }
      Color cardcolor;
      ...
      listItems.add(Container(
          ...
          child: Padding(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
              children: <Widget>[
                list[i].deal_newdate == null
                    ? Text("")
                    : Container(
                        width: widget.width / 10 * 3,
                        child: new Text(list[i].deal_newdate,
                            softWrap: false,
                            overflow: TextOverflow.ellipsis,
                            maxLines: 4,
                            textAlign: TextAlign.left),
                      ),
                list[i].hadlerlength == 0
                    ? list[i].createManager == 0
                        ? Container(width: widget.width / 10, child: Text(""))
                        : Container(
                            width: widget.width / 10,
                            child: Image.asset("assets/images/p20.png"))
                    : Container(
                        width: widget.width / 10,
                        child: Image.asset("assets/images/p21.png")),
                list[i].deal_messg == null
                    ? Text("")
                    : Container(
                        width: widget.width / 10 * 3,
                        child: new Text(list[i].deal_messg,
                            softWrap: false,
                            overflow: TextOverflow.ellipsis,
                            maxLines: 4,
                            textAlign: TextAlign.left),
                ...
                IconButton(
                  ...
                  icon: Image.asset("assets/images/p19.png"),
                  iconSize: 50,
                ),
              ],
            ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            style: ElevatedButton.styleFrom(backgroundColor: Color(0xff7588FA)),
            onPressed: () async {
              dynamic result =
                  await Navigator.pushNamed(context, '/messagefix');
            ...
            icon: Image.asset("assets/images/p18.png"),
            iconSize: 50,
          )
        ],
      ),
                      ...
                      : Column(
                          ...
                          children: [
                            show.deal_name == null
                                ? Text("")
                                : new Text(
                                    '回覆人員:\n' + show.handler_createManagerName,
                                    style: TextStyle(height: 1.5)),
                            SizedBox(
                              height: 15,
                            ),
                            show.handler_message == null
                                ? Text('')
